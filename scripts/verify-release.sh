...
find_python() {
  local candidates=()
  if [[ -n "${CIVICCONTRACTS_RELEASE_PYTHON:-}" ]]; then
    candidates+=("${CIVICCONTRACTS_RELEASE_PYTHON}")
  fi
...
exec((root / "civiccontracts" / "__init__.py").read_text(encoding="utf-8"), namespace)
...
dist = Path("dist")
wheel = dist / "civiccontracts-0.1.1-py3-none-any.whl"
sdist = dist / "civiccontracts-0.1.1.tar.gz"
